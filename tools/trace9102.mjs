...
game._preNhgetchHook = async () => {
    ...
    if (stepIdx >= lo && stepIdx <= hi) {
        console.log(`--- nhgetch#${stepIdx} moves=${game.moves} multi=${game.multi} occ=${JSON.stringify(game.occupation && (game.occupation.name||'fn'))} occtxt=${JSON.stringify(game.occtxt)} pendmsg=${JSON.stringify(game._pending_message)} more=${game._message_more} nomove=${game.nomove}`);
    }
    await origHook();
};
for (;;) {

// usage: node sessions-extra/findings/rng-diff-annotated.mjs <session.json> [startIdx] [count]
// Positional RNG diff keeping the C @callsite annotations.
...
const maxShow = Number(process.argv[4] || 12);
...
let shown = 0;
for (let i = startIdx; i < N && shown < maxShow; i++) {
  const c = normalizeRng(cFlat[i]?.call || '<none>');
  const j = normalizeRng(jsFlat[i] || '<none>');
  if (c !== j) {
    console.log(`mismatch @rng[${i}] step ${cFlat[i]?.step ?? '?'}:`);
    for (let k = Math.max(0, i - 4); k < Math.min(N, i + 14); k++) {
      const mark = k === i ? '>>' : '  ';
      console.log(`${mark}[${k}] step ${cFlat[k]?.step ?? '?'} C=${cFlat[k]?.call || '<none>'}  JS=${jsFlat[k] || '<none>'}`);
    }
    shown++;
    i += 13;
  }
}
if (!shown) console.log(`no positional mismatch in [${startIdx}..${N}) (C=${cFlat.length} JS=${jsFlat.length})`);
console.log(`totals: C=${cFlat.length} JS=${jsFlat.length}`);

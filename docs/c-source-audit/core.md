...
| `do.c` | `cmd.js`, `save.js`, `allmain.js` | `goto_level`, drop/floor effects, trap state, arrival timer interfaces. Earlier arrival overrides were removed. All nine callback kinds now use the shared queue, and level arrival runs it after object/monster delivery. The main turn still invokes it after the turn tail rather than at the end of `nh_timeout` before regions/regen; moving the clock increment and preserving prompt continuations require a separate live-turn change. Complete region/shop/holding order and unusual floor effects require further comparison. |
...
| `role.c` | `jsmain.js`, `allmain.js`, `mklev.js`, `cmd.js` | `str2role/str2race`, role selection and initialization interfaces. `roles.js` is a legacy stub with no live imports; do not count it as the runtime owner. Abbreviations, filters and race/gender/alignment combinations need systematic startup tests. |

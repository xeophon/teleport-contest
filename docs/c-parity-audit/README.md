...
1. Shop ledger foundation from `05`: current JS is still partly field-based; split-stack unpaid returns, itemized bill-row payment, accepted shop-floor container put-in sales, ordinary shop-floor magic-bag tip loss/put-in explosion billing, ordinary dropped-container sale/no-charge state, direct gold drop/pickup donation/charge parity, and broader angry/robbed `sellobj()` shopkeeper-state branches are covered in ordinary paths, but broad split routing, carried/non-ordinary magic-bag loss/destruction, full `obfree()`/container-aware `stolen_value()` parity, special-stock/uninterested polish, and `costly_alteration` coverage are missing.
...
- Recursive container put-in, take-out, tip moves, contained gold, accepted put-in sales, ordinary shop-floor magic-bag tip loss/put-in explosion billing, ordinary dropped-container sale/no-charge state, direct ordinary gold drop/pickup handling, and broader angry/robbed `sellobj()` handling are in place; next extend them through carried/non-ordinary magic-bag loss/destruction, lift limits, merge/destruction edge cases, and full `sellobj`/`subfrombill` routing.

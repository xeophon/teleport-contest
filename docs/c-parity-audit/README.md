...
1. Shop ledger foundation from `05`: current JS is still partly field-based; split-stack unpaid returns, itemized bill-row payment, and accepted shop-floor container put-in sales are started, but broad split routing, remaining container `sellobj`, and `costly_alteration` coverage are missing.
...
- Recursive container put-in, take-out, tip moves, contained gold, and accepted put-in sales are started; next extend them through magic-bag loss/destruction, lift limits, and full `sellobj`/`subfrombill` routing.
- Expand `sellobj()` beyond ordinary paid non-container drops and accepted shop-floor container put-in to ordinary gold drops, dropped-container sale state, robbed-shop, and angry-shopkeeper edge cases.

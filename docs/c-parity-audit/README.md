...
- Recursive container put-in, take-out, tip moves, and contained gold are started; next extend them through accepted sale prompts, magic-bag loss/destruction, lift limits, and full `sellobj`/`subfrombill` routing.
- Expand `sellobj()` beyond ordinary paid non-container drops to ordinary gold drops, accepted sale prompts, robbed-shop, and angry-shopkeeper edge cases.

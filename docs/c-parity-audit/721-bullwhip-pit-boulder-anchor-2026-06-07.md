...
- `nethack-c/upstream/src/apply.c:3093-3103` lets a spotted big monster override those anchors; if a monster exists and no anchor is selected, C falls through to the normal `whipattack` monster branch.
...
- The implementation keeps the existing monster branch as the fallback when there is a monster but no pit anchor, matching C's `goto whipattack` shape for small/medium targets without boulder or furniture. Audit 722 adds occupied-anchor relocation and spotted big-monster override.
...
- Exact `enexto()` destination selection and RNG remain approximated by the shared JS `enextoMonsterSpot()` helper for monster-occupied anchors.
- `teleds(..., TELEDS_ALLOW_DRAG)` side effects are represented by direct same-level relocation for ordinary pit-anchor cases; ball-and-chain drag, post-relocation trap effects, and vision details remain broader work.

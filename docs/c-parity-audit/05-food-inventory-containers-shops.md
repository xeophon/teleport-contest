...
- Carried/non-ordinary loss sources/targets and destroyed recursive contents can still miss full C `obfree()` preservation and `stolen_value()` debt naming; carried in-shop held/blast debt conversion, off-shop held/blast used-up preservation, failed spellbook reads, carried fire destruction, carried fire ignition, cursed wand backfires, hero-caused floor fire, and selected scatter/useup preservation are covered through local helpers, not a general C-shaped subsystem.

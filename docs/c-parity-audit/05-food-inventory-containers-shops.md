...
- `js/cmd.js:33688-33774`: drop command and unpaid-drop shop hook.
- `js/cmd.js:37510-37579`, `js/cmd.js:37704-37825`: fired and thrown projectile split/landing paths.
...
- `js/cmd.js:12804-13325`: starter shop bill entries, split/subtract helpers, carried projectile bill split, same-shop unpaid projectile return, pickup bill merge helpers, unpaid drop return, and test exports.
- `js/cmd.js:13337-13353`: buried-merchandise debt helper.
- `js/cmd.js:13714-13721`: unpaid line synchronization.
- `js/cmd.js:14135-14158`: used-up bill naming and memory helper.
- `js/cmd.js:18011-18021`: unpaid charged-tool usage fee helper.
- `js/cmd.js:18023-18056`: temporary floor source billing for special tip sources.
- `js/cmd.js:18134-18156`: horn-created object billing and unpaid stack merge helper.
- `js/cmd.js:18384-18674`: shop base cost, price calculation, debt collection, and debit payment.
- `js/cmd.js:25068-25166`: shop quote and pay menu handling.
- `js/cmd.js:38630-38666`: pay command source selection.
...
- Split-stack unpaid returns have starter `splitbill`/`subfrombill` helpers, partial inventory use preserves C's `bquan > quan` representation, and throw/fire projectile splits now create child bill rows and return same-shop landings. Stolen-value conversion for projectiles leaving the owning shop, bill-aware floor stacking, containers, and full recursive `subfrombill()`/`sellobj()` compatibility remain incomplete.
...
JS uses starter bill entries plus object fields (`unpaid`, `unpaidPrice`), loose shopkeeper counters/debit, and a global `_usedUpShopBills` list (`js/cmd.js:12804-13325`, `js/cmd.js:13714-13721`, `js/cmd.js:18639-18674`). This supports visible unpaid suffixes, ordinary pickup/drop billing, compatible pickup stack merges, narrow split-stack unpaid returns, partial inventory-use debt display, throw/fire child bill rows, and basic payment, but does not yet preserve C's full ownership and bill-entry semantics.

Concrete gaps:

- Ledger coverage is partial: starter `bill` entries and narrow split/subtract helpers exist, but there is no full source-equivalent `onbill()`, recursive `subfrombill()`, `stolen_value()`, or `obfree()`; projectile theft/debit conversion and merges outside ordinary pickup can still lose bill state.
...
- `collectPayableShopDebts()` scans inventory and floor objects globally, so payable objects are not strictly tied to a specific shopkeeper's bill (`js/cmd.js:18639-18662`).
...
- `get_cost()` parity is incomplete: JS has base tables, unknown-name surcharge, enchantment surcharge, charisma adjustment, and pricing units (`js/cmd.js:18384-18611`), but not full C role/status/shopkeeper anger/tourist/dunce/artifact/contained/no-charge/price-quote side effects.
...
4. Port pickup/drop around shop and stack invariants.
   ...
   - Finish projectile shop resolution with stolen-value debit/robbed conversion and bill-aware floor stacking, then continue into container flows.
...
- Multi-pickup still lacks full C quote/lift semantics, and split-stack object identity can still lose unpaid/shop state in projectile theft/debit and non-pickup merge paths.

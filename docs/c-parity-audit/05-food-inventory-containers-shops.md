...
- Payment now applies shop credit before cash for selected ledger rows, shrinks partly used bills before intact rows can clear them, aggregates ledger-backed carried-container and floor-container contents under one payable line, rejects a partly-used contained live residual until the used-up row is selected or already paid, clears every constituent bill row, processes queued selected rows in C itemized order, stops after the first selected row that has become unaffordable while preserving earlier purchases, and handles robbed-only `#pay` compensation with C's after-blood messaging and half-loss threshold. Legacy container itemized payment and remaining prompt/menu edge cases still do not match C.
...
- Payment is only partly backed by authoritative bill rows; ledger-backed carried and floor container contents now aggregate for `#pay`, including partly-used contained-item rejection/order and queued selected-row ordering/partial-affordability stops, but legacy unpaid fallbacks and remaining itemized prompt/menu edge cases still need C parity.

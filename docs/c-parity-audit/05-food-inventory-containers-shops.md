...
- Lift limits are not full C parity: starter pre-transfer slot/max-carry failure, artifact/fatal-corpse touch gates, floor Rider corpse revival, single-object and menu burden prompts, partial stack pickup, gold lift splitting, scare-monster scroll pickup state/dusting, loadstone weight/slot exceptions, and multi-pickup partial-success ordering exist for ordinary floor pickup, but boulder handling, scare-scroll call/quote/message polish, and the full artifact bane/silver/life-saving matrix remain incomplete.

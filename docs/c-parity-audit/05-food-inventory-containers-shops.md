...
- `js/cmd.js:24588-24674`, `js/cmd.js:28361-28432`: multi-object pickup menu and sequential selected-object pickup, including C-style partial-success ordering where earlier successful objects remain picked up when a later selected object fails, plus menu scare-monster scroll state/dust handling.
...
- `js/cmd.js:17011-17264`, `js/cmd.js:28361-28432`, `js/cmd.js:42384-42484`: floor pickup now handles scare-monster scroll identity, blessed/uncursed/used state changes, dusting, and used-up shop billing before ordinary inventory insertion; menu pickup treats dusting as success-and-continue and preserves partial-stack remainder state.
...
- Lift limits are not full C parity: starter pre-transfer slot/max-carry failure, artifact/fatal-corpse touch gates, floor Rider corpse revival, single-object burden prompts, partial stack pickup, gold lift splitting, scare-monster scroll pickup state/dusting, loadstone weight/slot exceptions, and multi-pickup partial-success ordering exist for ordinary floor pickup, but multi-pickup burden and gold-lift semantics, boulder handling, scare-scroll call/quote/message polish, and the full artifact bane/silver/life-saving matrix remain incomplete.
...
- Multi-pickup still lacks full C quote, burden-prompt, and gold-lift semantics, though selected-object partial-success ordering and scare-scroll state/dust basics are now C-shaped; split-stack object identity can still lose unpaid/shop state in non-projectile delete, container, and generic merge paths.

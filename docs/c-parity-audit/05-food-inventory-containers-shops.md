...
- `js/cmd.js:13380-13422`, `js/cmd.js:17098-17366`, `js/cmd.js:22485-22820`, `js/cmd.js:28474-28503`, `js/cmd.js:28842-28898`, `js/cmd.js:29020-29036`, `js/cmd.js:34471-34486`, `js/cmd.js:42384-42484`, `js/cmd.js:42756-42789`: floor pickup now handles scare-monster scroll identity, blessed/uncursed/used state changes, dusting, C-ordered type-call prompts before `useupf()`-style used-up shop billing, and ordinary inventory insertion; reading an unknown scare-monster scroll also enters the scroll type-call path; single shop-floor pickup runs lift/scare preflight and burden prompts before the JS shop quote; ordinary single/menu stack pickup quotes use C per-unit wording before inventory pickup lines; menu pickup treats dusting as success-and-continue and preserves partial-stack remainder state.
...
JS single pickup computes a shop price, runs preflight, then shows a shop quote before transferring the picked object (`js/cmd.js:42200-42508`, `js/cmd.js:42756-42789`). It now has a starter floor-pickup preflight before quote/transfer/billing for artifact blast/evasion/death, fatal barehanded cockatrice/chickatrice corpse touch, floor Rider corpse revival, maximum-carry failure, partial stack lifting, single-object burden prompts, gold lift splitting, inventory-slot failure, single-object scare-monster scroll handling including unknown-scroll type-call before used-up billing/deletion, loadstone weight/slot exceptions, and floor boulder lift/Sokoban/rock-throwing exceptions (`js/cmd.js:17000-17366`, `js/cmd.js:28684-28708`, `js/cmd.js:42302-42484`). Ordinary single and menu shop pickups now share C-style stack quote wording, using the unit price with `per <singular>` before the inventory line. Multi-pickup now displays C-style shop price suffixes in the selection menu and processes selected floor objects sequentially, running preflight and transfer against current inventory state for each selected object, so earlier successful picked shop objects remain carried and billed if a later selected object fails; boulder failures stop later selections while already-picked earlier selections remain, scare-monster scroll dusting in this path now prompts for an unknown type before billing/deletion and then continues to later selected items, partial live stacks restore the floor remainder's old state, per-item burden prompts can skip or abort the remaining selected list, and selected gold uses the same liftable-count split before shop charging (`js/cmd.js:22485-22820`, `js/cmd.js:28474-28503`, `js/cmd.js:28928-28954`).

Concrete gaps:

- Multi-object pickup now has priced menu rows and ordinary selected-object shop quotes before inventory lines; remaining quote/display gaps are scare-scroll floor-style dust quotes, container/contents quote edge cases, and the full source `addtobill()` positioning across less ordinary transfer paths.
...
- Lift limits are not full C parity: starter pre-transfer slot/max-carry failure, artifact/fatal-corpse touch gates, floor Rider corpse revival, single-object and menu burden prompts, partial stack pickup, gold lift splitting, scare-monster scroll pickup state/dusting/type-call ordering, loadstone weight/slot exceptions, floor/container boulder lift/Sokoban/rock-throwing exceptions, priced pickup menu rows, ordinary pickup quote wording, and multi-pickup partial-success ordering exist for ordinary floor pickup and container take-out, but scare-scroll floor-style quote polish, container/contents quote edge cases, and the full artifact bane/silver/life-saving matrix remain incomplete.
...
- Used-up unpaid items are not centralized. C `useup()`/`obfree()` preserve bills for consumed unpaid objects; JS only remembers selected corpse rot/glob shrink, projectile, payment, magic-bag destruction, failed spellbook read, carried fire destruction, hero-caused floor fire, and scare-scroll pickup dust paths.
...
- Multi-pickup now has ordinary priced menu rows and stack-aware shop pickup quotes, plus selected-object partial-success ordering, burden prompts, gold lift preflight, and scare-scroll state/dust/type-call ordering. Remaining visible pickup risk is scare-scroll floor-style dust quote polish and container/contents quote edge cases. Carried-bag partial unpaid put-in now preserves split child bill rows, but split-stack object identity can still lose unpaid/shop state in non-projectile delete, other container, and generic merge paths.
- Container-moving objects in shops and complex drops still do not exercise full C `sellobj()`/`addtobill()`/`subfrombill()` paths outside ordinary dropped-container, angry/robbed and special-stock/uninterested `sellobj()` branches, ordinary gold drop/pickup paths, ordinary shop-floor magic-bag tip loss/put-in explosion slices, narrow carried magic-bag held-loss billing, and magic-bag scatter/useup plus destructive floor-effect preservation; generic merge/destruction paths and shop-floor cursed magic-bag `#loot` loss remain high-risk.

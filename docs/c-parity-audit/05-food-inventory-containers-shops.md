...
- `js/cmd.js:22485-22627`, `js/cmd.js:28474-28503`, `js/cmd.js:28928-28954`: multi-object pickup menu and sequential selected-object pickup, including C-style partial-success ordering where earlier successful objects remain picked up when a later selected object fails, menu scare-monster scroll state/dust handling, per-item burden prompts, and menu gold lift preflight.
...
- `js/cmd.js:13380-13422`, `js/cmd.js:17098-17366`, `js/cmd.js:22485-22627`, `js/cmd.js:28474-28503`, `js/cmd.js:28842-28898`, `js/cmd.js:29020-29036`, `js/cmd.js:34471-34486`, `js/cmd.js:42384-42484`, `js/cmd.js:42756-42789`: floor pickup now handles scare-monster scroll identity, blessed/uncursed/used state changes, dusting, C-ordered type-call prompts before `useupf()`-style used-up shop billing, and ordinary inventory insertion; reading an unknown scare-monster scroll also enters the scroll type-call path; single shop-floor pickup runs lift/scare preflight and burden prompts before the JS shop quote; menu pickup treats dusting as success-and-continue and preserves partial-stack remainder state.
...
JS single pickup computes a shop price, runs preflight, then shows the JS shop quote before transferring the picked object (`js/cmd.js:42200-42508`, `js/cmd.js:42756-42789`). It now has a starter floor-pickup preflight before quote/transfer/billing for artifact blast/evasion/death, fatal barehanded cockatrice/chickatrice corpse touch, floor Rider corpse revival, maximum-carry failure, partial stack lifting, single-object burden prompts, gold lift splitting, inventory-slot failure, single-object scare-monster scroll handling including unknown-scroll type-call before used-up billing/deletion, loadstone weight/slot exceptions, and floor boulder lift/Sokoban/rock-throwing exceptions (`js/cmd.js:17000-17366`, `js/cmd.js:28684-28708`, `js/cmd.js:42302-42484`). Multi-pickup now processes selected floor objects sequentially, running preflight and transfer against current inventory state for each selected object, so earlier successful picked shop objects remain carried and billed if a later selected object fails; boulder failures stop later selections while already-picked earlier selections remain, scare-monster scroll dusting in this path now prompts for an unknown type before billing/deletion and then continues to later selected items, partial live stacks restore the floor remainder's old state, per-item burden prompts can skip or abort the remaining selected list, and selected gold uses the same liftable-count split before shop charging (`js/cmd.js:22485-22627`, `js/cmd.js:28474-28503`, `js/cmd.js:28928-28954`).

Concrete gaps:

- Multi-object pickup still bypasses the exact C quote/menu-display flow; ordinary picked objects now enter the JS shop ledger, selected objects run sequentially for C-style partial-success ordering, scare-monster scrolls use the state/dust basics, menu burden prompts use C's skip-versus-abort result split, selected shop gold is split before charging, and loadstones and boulders use their C weight exceptions in this path.
...
- Lift limits are not full C parity: starter pre-transfer slot/max-carry failure, artifact/fatal-corpse touch gates, floor Rider corpse revival, single-object and menu burden prompts, partial stack pickup, gold lift splitting, scare-monster scroll pickup state/dusting/type-call ordering, loadstone weight/slot exceptions, floor/container boulder lift/Sokoban/rock-throwing exceptions, and multi-pickup partial-success ordering exist for ordinary floor pickup and container take-out, but scare-scroll message polish, exact C `addtobill()` quote positioning, and the full artifact bane/silver/life-saving matrix remain incomplete.
...
- Multi-pickup still lacks exact C quote/menu-display polish, though selected-object partial-success ordering, burden prompts, gold lift preflight, and scare-scroll state/dust/type-call ordering are now C-shaped; single-object shop-floor pickup now gates the JS quote behind floor preflight, but successful quote text still is not routed through C's exact `addtobill()` position. Carried-bag partial unpaid put-in now preserves split child bill rows, but split-stack object identity can still lose unpaid/shop state in non-projectile delete, other container, and generic merge paths.

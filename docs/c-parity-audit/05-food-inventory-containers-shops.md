...
- Generic `costly_alteration()` is still partial. JS now covers unpaid food `COST_BITE`, narrow tin `COST_OPEN/COST_DSTROY`, non-destroying box-lock `COST_BRKLCK`, cream-pie `COST_SPLAT`, and cursed-charging `COST_UNCHRG`/`COST_DECHNT` carried-object paths, plus local billing for buried merchandise and horn-created objects, but not the shared open/destroy/cancel/degrade billing hook used across C.
...
- Tin paths outside the narrow open/trap-destroy and basic tinning-kit billing slices plus generic non-bite alterations can still avoid C billing because full `costly_tin()` integration and shared non-bite `costly_alteration()` coverage beyond box-lock and cream-pie billing are still incomplete.

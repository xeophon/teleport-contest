...
- `collectPayableShopDebts()` now enumerates selected shopkeeper bill rows first and splits `bquan > quan` rows into used-up/intact portions, but retains object-field fallback scanning for legacy unpaid objects (`js/cmd.js:22598-22694`, `js/cmd.js:22795-22835`, `js/cmd.js:43567-43570`).
- Payment now applies shop credit before cash for selected ledger rows, shrinks partly used bills before intact rows can clear them, and handles robbed-only `#pay` compensation with C's after-blood messaging and half-loss threshold. Container itemized payment and full queued-selection behavior still do not match C.
...
- Payment is only partly backed by authoritative bill rows; container payment and several legacy unpaid fallbacks still need C parity.

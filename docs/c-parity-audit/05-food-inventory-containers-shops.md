...
- Shop-floor take-out now covers ordinary objects, moved-container recursive contents/contained gold, loose top-level gold, and pre-billing refusal for inventory-slot exhaustion or maximum-carry failure, but burden prompts, partial stack lifting, artifact/fatal-corpse checks, and bill-aware inventory merge edge cases remain incomplete.

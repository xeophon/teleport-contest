...
- `collectPayableShopDebts()` now enumerates selected shopkeeper bill rows first, splits `bquan > quan` rows into used-up/intact portions, and aggregates ledger-backed unpaid contents inside carried outer containers, but retains object-field fallback scanning for legacy unpaid objects (`js/cmd.js:22592-22739`, `js/cmd.js:22863-22941`, `js/cmd.js:43660-43663`).
- Payment now applies shop credit before cash for selected ledger rows, shrinks partly used bills before intact rows can clear them, aggregates ledger-backed carried-container contents under one payable line, and handles robbed-only `#pay` compensation with C's after-blood messaging and half-loss threshold. Floor/legacy/partly-used container itemized payment and full queued-selection behavior still do not match C.
...
- Payment is only partly backed by authoritative bill rows; container payment outside ledger-backed carried contents and several legacy unpaid fallbacks still need C parity.

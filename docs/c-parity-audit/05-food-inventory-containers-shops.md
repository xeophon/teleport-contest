...
- Generic `costly_alteration()` is still partial. JS now covers unpaid food `COST_BITE`, narrow tin `COST_OPEN/COST_DSTROY`, and non-destroying box-lock `COST_BRKLCK` paths, plus local billing for buried merchandise and horn-created objects, but not the shared open/destroy/cancel/degrade billing hook used across C.
- Used-up unpaid items are not centralized. C `useup()`/`obfree()` preserve bills for consumed unpaid objects; JS only remembers selected corpse rot/glob shrink, projectile, payment, magic-bag destruction, failed spellbook read, carried fire destruction, hero-caused floor fire, and scare-scroll pickup dust paths with floor-style billing quotes.
...
- Tin paths outside the narrow open/trap-destroy and basic tinning-kit billing slices plus generic non-bite alterations can still avoid C billing because full `costly_tin()` integration and shared non-bite `costly_alteration()` coverage beyond box-lock billing are still incomplete.

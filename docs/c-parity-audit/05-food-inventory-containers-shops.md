...
- `js/cmd.js:19554-20145`: container put-in menus, rejection checks, and transfer helpers.
- `js/cmd.js:15542-15934`, `js/cmd.js:16810-17035`, `js/cmd.js:20194-20388`, `js/cmd.js:38505-38570`: container take-out shop billing, pre-lift touch/weight checks, preparation, and inventory insertion.
- `js/cmd.js:20434-20613`, `js/cmd.js:38821-38844`: tip checks and destination selection.
...
JS has a substantial UI implementation for boxes, bags, ice boxes, take-out, put-in, stash, and tip (`js/cmd.js:19554-20613`, `js/cmd.js:38049-38844`). It also implements some magic bag, bag of tricks, and horn of plenty behavior. Ordinary take-out from a shop-floor container now goes through a starter `addtobill()`-style helper, prices from the source container's coordinates, skips carried containers/already-unpaid objects, requires the source container to be a level-floor object, clears ordinary `no_charge` contents when they are taken back without billing, routes loose gold through C-shaped `costly_gold()` credit/debit/loan transitions, runs `out_container()`/`lift_object()`-ordered preflight so self-willed quest artifact evasion, fatal barehanded cockatrice/chickatrice corpse touch, slot exhaustion, maximum-carry failures, burden prompts, declined burden prompts, and overweight stack splits happen before extraction, recursive billing, gold debt, or inventory insertion, and can merge stackable no-charge or same-price unpaid contents into compatible carried stacks without requiring a fresh inventory letter. Ordinary put-in now has starter `sellobj()`-style coverage for unpaid non-container returns, partial unpaid split rows, outside-shop debt preservation, no-sale/full-bill uninterested handling with `no_charge` marking, accepted paid-object/container sale prompts with cash/credit handling, and loose-gold `donate_gold()` credit/debt handling, with container stacking guarded against unpaid/no-charge provenance loss (`js/mklev.js:4681-4701`). Ordinary tip now bills each top-level object from an actual shop-floor source before floor or carried-target placement; loose gold tipped to the floor also runs the C `dropy()`/`sellobj()` donation offset, while loose gold tipped into a carried container only applies the `costly_gold()` charge. It now also recurses into moved containers for contained contents/gold where the source C path uses `contained_cost()`, `contained_gold()`, `addtobill()`, or `sellobj()`, ordinary shop-floor magic-bag tip loss/put-in explosion paths preserve used-up bill rows for destroyed bags/triggers and charge vanished merchandise while consuming shop credit, narrow carried magic-bag held losses convert top-level unpaid vanished contents to debt while preserving nested unpaid contents in non-unpaid vanished containers as used-up rows, and magic-bag scatter breakage, consumed-on-hit objects, destructive floor-effect landings, trigger/target destruction, and scattered unpaid stack splits now preserve existing bill rows through used-up entries.
...
- Shop-floor take-out now covers ordinary objects, moved-container recursive contents/contained gold, loose top-level gold, pre-billing refusal for self-willed quest artifacts, fatal barehanded touch-petrifying corpse removal, inventory-slot exhaustion, or maximum-carry failure, burden prompts, partial stack lifting, and stackable no-charge/same-price unpaid inventory merges, but full artifact blast/damage side effects and less common inventory overflow/merge quirks remain incomplete.
...
- Take-out has a starter C `out_container()`/`lift_object()`-ordered preflight for self-willed quest artifact evasion, fatal barehanded touch-petrifying corpse removal, slots, maximum-carry failure, burden prompts, and partial stack lifting; full artifact touch blast/damage side effects and uncommon inventory overflow/merge paths still need follow-up.

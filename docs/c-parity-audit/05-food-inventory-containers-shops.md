...
- `check_unpaid_usage()` is still partial. JS now has a reusable debit-only helper with C-style charged-object fee branches and callers for bag/horn use, wand zaps, camera use, can-of-grease applications, normal lamp and potion-of-oil apply lighting, carried fire ignition of unpaid lamps/light sources, magic-lamp `#rub` djinni release, successful spellbook study completion, magic-marker writing, tinning-kit corpse applications, crystal-ball gazing, magic flute/harp improvisation, frost/fire horn direction zaps, alternate emptying, and drum-of-earthquake charges; the ordinary musical-instrument shell shares the same C gates while correctly avoiding usage billing for no-charge mundane instruments. Other charge-consuming callers, such as wand engraving, still need to route through the appropriate C billing paths.

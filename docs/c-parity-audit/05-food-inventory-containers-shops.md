...
- Payment is only partly backed by authoritative bill rows; ledger-backed carried and floor container contents now aggregate for `#pay`, including partly-used contained-item rejection/order, queued selected-row ordering/partial-affordability stops, and used-up/unpaid menu headings, but legacy unpaid fallbacks and remaining itemized prompt edge cases still need C parity.

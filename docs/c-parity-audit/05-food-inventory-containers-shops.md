...
- Tip now covers ordinary per-item shop billing for non-gold contents, but magic-bag loss/explosion, contained gold, recursive container contents, and destroyed-item debt naming still do not fully match C `tipcontainer()`/`stolen_value()`.

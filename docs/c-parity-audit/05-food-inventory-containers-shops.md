...
- `js/cmd.js:17011-17264`, `js/cmd.js:22485-22627`, `js/cmd.js:28474-28503`, `js/cmd.js:29020-29036`, `js/cmd.js:42384-42484`, `js/cmd.js:42756-42789`: floor pickup now handles scare-monster scroll identity, blessed/uncursed/used state changes, dusting, and used-up shop billing before ordinary inventory insertion; single shop-floor pickup runs lift/scare preflight and burden prompts before the JS shop quote; menu pickup treats dusting as success-and-continue and preserves partial-stack remainder state.
...
JS single pickup computes a shop price, runs preflight, then shows the JS shop quote before transferring the picked object (`js/cmd.js:42200-42508`, `js/cmd.js:42756-42789`). It now has a starter floor-pickup preflight before quote/transfer/billing for artifact blast/evasion/death, fatal barehanded cockatrice/chickatrice corpse touch, floor Rider corpse revival, maximum-carry failure, partial stack lifting, single-object burden prompts, gold lift splitting, inventory-slot failure, single-object scare-monster scroll handling, loadstone weight/slot exceptions, and floor boulder lift/Sokoban/rock-throwing exceptions (`js/cmd.js:17000-17336`, `js/cmd.js:28684-28708`, `js/cmd.js:42302-42484`). Multi-pickup now processes selected floor objects sequentially, running preflight and transfer against current inventory state for each selected object, so earlier successful picked shop objects remain carried and billed if a later selected object fails; boulder failures stop later selections while already-picked earlier selections remain, scare-monster scroll dusting in this path now continues to later selected items, partial live stacks restore the floor remainder's old state, per-item burden prompts can skip or abort the remaining selected list, and selected gold uses the same liftable-count split before shop charging (`js/cmd.js:22485-22627`, `js/cmd.js:28474-28503`, `js/cmd.js:28928-28954`).
...
- Lift limits are not full C parity: starter pre-transfer slot/max-carry failure, artifact/fatal-corpse touch gates, floor Rider corpse revival, single-object and menu burden prompts, partial stack pickup, gold lift splitting, scare-monster scroll pickup state/dusting, loadstone weight/slot exceptions, floor/container boulder lift/Sokoban/rock-throwing exceptions, and multi-pickup partial-success ordering exist for ordinary floor pickup and container take-out, but scare-scroll `trycall()`/message polish, exact C `addtobill()` quote positioning, and the full artifact bane/silver/life-saving matrix remain incomplete.

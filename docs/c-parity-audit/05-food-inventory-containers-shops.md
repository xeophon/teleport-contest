...
- Shop-floor put-in now covers ordinary unpaid non-container returns, recursive moved-container contents/contained gold, accepted paid-object/container sale prompts, broader angry/robbed `sellobj()` shopkeeper-state handling, no-sale `no_charge` marking, loose-gold donation/credit, and ordinary shop-floor magic-bag explosion billing for destroyed target bags, unpaid triggers, vanished contents, and scatter/useup plus destructive floor-effect preservation, but non-ordinary magic-bag source/target handling and shared-helper integration remain incomplete.
...
- `check_unpaid_usage()` is still partial. JS now has a reusable debit-only helper with C-style charged-object fee branches and callers for bag/horn use, wand zaps, camera use, can-of-grease applications, normal lamp and potion-of-oil apply lighting, magic-lamp `#rub` djinni release, successful spellbook study completion, alternate emptying, and drum-of-earthquake charges, but remaining marker, tinning-kit, and other charge-consuming callers still need to route through the appropriate C billing paths.
...
- Carried/non-ordinary loss sources/targets and destroyed recursive contents can still miss full C `obfree()` preservation and `stolen_value()` debt naming; carried held/blast-loss billing, failed spellbook reads, carried fire destruction, and selected scatter/useup preservation are covered through local helpers, not a general C-shaped subsystem. Hero-caused floor fire still lacks C `useupf()` billing.

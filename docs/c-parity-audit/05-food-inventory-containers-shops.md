...
- Used-up unpaid items are not centralized. C `useup()`/`obfree()` preserve bills for consumed unpaid objects; JS only remembers selected corpse rot/glob shrink, projectile, payment, magic-bag destruction, failed spellbook read, carried fire destruction, cream-pie splats, hero-caused floor fire, and scare-scroll pickup dust paths with floor-style billing quotes.

...
- Shop-floor take-out now covers ordinary objects, moved-container recursive contents/contained gold, loose top-level gold, pre-billing refusal for inventory-slot exhaustion or maximum-carry failure, burden prompts, partial stack lifting, and stackable no-charge/same-price unpaid inventory merges, but artifact/fatal-corpse checks and less common inventory overflow/merge quirks remain incomplete.
...
- Take-out has a starter C `lift_object()`-ordered preflight for slots, maximum-carry failure, burden prompts, and partial stack lifting; it still does not cover artifact touch or fatal-corpse checks.

...
- `collectPayableShopDebts()` now enumerates selected shopkeeper bill rows first, splits `bquan > quan` rows into used-up/intact portions, and aggregates ledger-backed unpaid contents inside carried or shop-floor outer containers, but retains object-field fallback scanning for legacy unpaid objects (`js/cmd.js:22592-22739`, `js/cmd.js:22863-22941`, `js/cmd.js:43660-43663`).
- Payment now applies shop credit before cash for selected ledger rows, shrinks partly used bills before intact rows can clear them, aggregates ledger-backed carried-container and floor-container contents under one payable line, clears every constituent bill row, and handles robbed-only `#pay` compensation with C's after-blood messaging and half-loss threshold. Legacy container itemized payment, partly-used contained-item rejection, and full queued-selection behavior still do not match C.
...
- Payment is only partly backed by authoritative bill rows; ledger-backed carried and floor container contents now aggregate for `#pay`, but legacy unpaid fallbacks, partly-used contained-item rejection, and queued itemized behavior still need C parity.

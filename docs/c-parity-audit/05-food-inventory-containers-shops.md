...
- `collectPayableShopDebts()` now enumerates selected shopkeeper bill rows first, splits `bquan > quan` rows into used-up/intact portions, keeps partly-used live residuals inside carried or shop-floor containers folded into the container row, and aggregates other ledger-backed unpaid contents inside carried or shop-floor outer containers, but retains object-field fallback scanning for legacy unpaid objects (`js/cmd.js:22592-22739`, `js/cmd.js:22830-22941`, `js/cmd.js:43660-43663`).
- Payment now applies shop credit before cash for selected ledger rows, shrinks partly used bills before intact rows can clear them, aggregates ledger-backed carried-container and floor-container contents under one payable line, rejects a partly-used contained live residual until the used-up row is selected or already paid, clears every constituent bill row, and handles robbed-only `#pay` compensation with C's after-blood messaging and half-loss threshold. Legacy container itemized payment and full queued-selection behavior still do not match C.
...
- Payment is only partly backed by authoritative bill rows; ledger-backed carried and floor container contents now aggregate for `#pay`, including partly-used contained-item rejection/order, but legacy unpaid fallbacks and queued itemized behavior still need C parity.

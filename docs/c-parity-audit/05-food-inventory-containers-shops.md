...
- Carried/non-ordinary loss sources/targets and destroyed recursive contents can still miss full C `obfree()` preservation and `stolen_value()` debt naming; carried held/blast-loss billing, failed spellbook reads, carried fire destruction, cursed wand backfires, hero-caused floor fire, and selected scatter/useup preservation are covered through local helpers, not a general C-shaped subsystem.

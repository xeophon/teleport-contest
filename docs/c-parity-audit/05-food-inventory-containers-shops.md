...
- Used-up unpaid items are not centralized. C `useup()`/`obfree()` preserve bills for consumed unpaid objects; JS only remembers selected corpse rot/glob shrink, projectile, payment, magic-bag destruction, failed spellbook read, carried fire destruction and carried fire ignition of unpaid light sources, cream-pie splats, cursed wand backfires, hard-landing fragile container contents, hero-caused floor fire, and scare-scroll pickup dust paths with floor-style billing quotes.
...
4. Port pickup/drop around shop and stack invariants.
   ...
   - Broaden projectile shop resolution for thrown-object breakage, kicking/ship/pick special cases, and remaining `check_shop_obj()`/`sellobj()` branches, then continue into container flows.

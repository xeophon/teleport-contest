...
- Generic `costly_alteration()` is still partial. JS now covers unpaid food `COST_BITE`, narrow tin `COST_OPEN/COST_DSTROY`, non-destroying box-lock `COST_BRKLCK`, and cream-pie `COST_SPLAT` paths, plus local billing for buried merchandise and horn-created objects, but not the shared open/destroy/cancel/degrade billing hook used across C.

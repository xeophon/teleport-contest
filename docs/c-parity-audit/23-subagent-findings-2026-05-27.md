...
- Later audits cover water, oiling weapons, acid/corrosion, poison coating, unicorn horn/amethyst mixtures, broad non-self carried potion menus, bounded polymorph dipping, potion-potion alchemy recipes/bad mixtures, alchemy-explosion vapor effects, and hard-landing projectile broken-potion vapor effects. Remaining generic potion work is direct `potionhit()` thrown/bash delivery, other broken-potion callers, non-`kn` `trycall()` prompt parity, water vapor gremlin/lycanthropy transformations, full `poly_obj()` fidelity, real `?*` menu rendering, self-potion/Klein-bottle handling, exact status-property mapping, and shared damage/discovery primitives.

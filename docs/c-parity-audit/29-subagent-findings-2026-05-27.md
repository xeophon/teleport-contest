...
- Remaining potion matrix work is direct `potionhit()` thrown/bash delivery, other broken-potion callers, non-`kn` `trycall()` prompt parity, water vapor gremlin/lycanthropy transformations, full `poly_obj()` fidelity, shared water-damage/discovery primitives, real `?*` menu rendering, self-potion/Klein-bottle handling, exact status-property mapping, and poison lifecycle outside dipping; horn/amethyst, water, broad menus, unsupported no-effect pairs, bounded polymorph dipping, potion-potion alchemy recipes/bad mixtures, alchemy-explosion vapor effects, and hard-landing projectile broken-potion vapor effects are covered in audits 32-38.

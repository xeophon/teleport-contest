...
This slice closes two stale simple `FOOD(...)` merge omissions: `tripe ration` and `candy bar`. It is intentionally limited to ordinary pickup inventory merging, shop unpaid merge proof, C unit price coverage, and the no-charge full-inventory merge preflight. It does not add a full object metadata registry; `meat stick` object identity is covered later in audit 27.
...
- `nethack-c/upstream/include/objects.h:1054-1064`: meatball and meat stick are mergeable `FOOD(...)` rows, while `meat ring` is explicitly non-mergeable. This audit left meat stick for a follow-up identity slice, now documented in audit 27.

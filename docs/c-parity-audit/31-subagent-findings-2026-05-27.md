...
- Broad non-self carried potion `#dip` source/target menu parity, neutral-water damage, horn/amethyst mixtures, and potion-potion alchemy recipes/bad mixtures are covered in later audits. Real `?*` menu rendering, full `poly_obj()` fidelity, self-potion/Klein-bottle handling, `potionbreathe()` side effects, and poison lifecycle outside dipping remain separate slices.

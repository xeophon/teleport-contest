...
- `empty` now zeroes bag-of-tricks and horn-of-plenty charges, the wish path refreshes final `owt` after qualifiers and quantity, and lenses now use C's weight 3, cost 80, `oc_prob + 1` namedesc bound, non-merge quantity behavior, and "pair of lenses" display. This still depends on local weight/display maps rather than the C `objects[]` table.

...
- `applyWishedQuantity` now follows the C shape for common cases: non-mergeable wishes such as boots, wands, meat rings, magic markers, and spellbooks stay at one, plural spellings resolve to the base object where covered, pancake/cram/kelp/royal-jelly/K/C ration plural wishes get C-style two-object quantities, and candles/ammunition keep the C multigen caps. It is still local policy rather than full `objects[].oc_merge` metadata.
...
- replace the local weight maps behind final wished-object `owt` with registry-backed `weight()`; pancake/cram/kelp/royal-jelly/K/C ration weights are covered in the local overlay, but the broader registry is still missing

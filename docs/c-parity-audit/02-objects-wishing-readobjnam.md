...
- `applyWishedQuantity` now follows the C shape for common cases: non-mergeable wishes such as boots, wands, meat rings, magic markers, and spellbooks stay at one, plural spellings resolve to the base object where covered, and candles/ammunition keep the C multigen caps. It is still local policy rather than full `objects[].oc_merge` metadata.

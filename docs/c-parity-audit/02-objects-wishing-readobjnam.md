...
- `applyWishedQuantity` now follows the C shape for common cases: non-mergeable wishes such as boots, wands, meat rings, expensive cameras, tinning kits, cans of grease, magic markers, bag of tricks, crystal balls, and spellbooks stay at one, plural spellings resolve to the base object where covered, pancake/cram/kelp/royal-jelly/meatball/enormous-meatball/K/C ration plural wishes get C-style two-object quantities, and candles/ammunition keep the C multigen caps. It is still local policy rather than full `objects[].oc_merge` metadata.
...
- selected non-wizard substitutions, wizard-mode real Candelabrum/Book name and description wishes, and the Bell namedesc silver-bell path exist around `js/cmd.js:11760`, but they are parser-specific and not backed by `objects[].oc_nowish`.
...
8. **`readobjnam` matching is still local.** Unrecognized wishes no longer become arbitrary named weapons, exact `nothing`/`nil`/`none` declines now stay distinct from no-match retry, and repeated failures randomize through the existing C-shaped path. JS still lacks C `wishymatch`, ranges, namedesc lookup, alternate spellings, wizard terrain/trap/furniture and denied-artifact non-object results, and full parser staging.
...
5. return an explicit result kind: object, terrain/trap/furniture wizard wish, hands/nothing, or no match; the no-match and exact no-wish sentinels exist, but the other result kinds still need the full C split
...
- registry-backed non-wizard substitutions for real Amulet, Candelabrum, Bell, Book, magic lamp, and `oc_nowish`, preserving the currently covered parser-local name/description cases

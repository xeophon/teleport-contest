...
Figurine wishes now apply C's requested monster-type restrictions for unique monsters, ordinary human/non-were monsters, werecreature exceptions, and mail daemon exclusion while retaining the normal random initialized figurine when the requested monster is disallowed (`nethack-c/upstream/src/objnam.c:5191-5240`, `nethack-c/upstream/src/mkobj.c:1040-1046`).

Corpse, statue, tin, and egg wishes now cover the adjacent C binding differences. The JS path creates the normal randomized base object first, converts non-figurine werebeasts to human were-forms, preserves randomized fallback corpses/tins when a requested species is disallowed, lets statues bind requested human/unique/no-corpse monsters, applies tin unique/no-corpse/genocide/nutrition gates, and routes explicit eggs through C-shaped hatchability including Scorpius-to-scorpion and baby-to-grown mappings (`nethack-c/upstream/src/objnam.c:5191-5244`, `nethack-c/upstream/src/mon.c:5540-5587`, `nethack-c/upstream/src/mkobj.c:898-934`).
...
- Figurine requested-monster restrictions now cover C's `G_UNIQ`, `M2_HUMAN` unless `M2_WERE`, and mail-daemon exclusion rule in the wish-local path; broader figurine factory, random-selection, transform-timer, and registry-backed monster metadata parity remains open.
- Corpse/statue/tin/egg requested-monster binding now covers the local C distinctions for wereform conversion, corpse/tin fallback preservation, permissive statue binding, tin species gates, and invalid egg hatch-timer clearing; broader corpstat/tin/egg factory metadata and shared timer parity remain open.
- `empty` now zeroes bag-of-tricks and horn-of-plenty charges, the wish path refreshes final `owt` after qualifiers and quantity, lenses now use C's weight 3, cost 80, `oc_prob + 1` namedesc bound, non-merge quantity behavior, and "pair of lenses" display, meat rings use C food identity/weight 5 after plural lookup, candles use weight 2 per unit, wished horns of plenty keep the concrete `HORN_OF_PLENTY` type, wished bag of tricks, expensive camera, tinning kit, can of grease, magic marker, crystal ball, magic flute, frost horn, fire horn, horn of plenty, magic harp, drum of earthquake, and the Bell of Opening path now share local C metadata or equivalent special-row handling for non-merge quantity, charged status, unit weight/cost, and wish-time charge caps, and wished pancake/cram/kelp/slime-mold/royal-jelly/meatball/enormous-meatball/K/C ration rows carry concrete C food identity, capitalization, weights, nutrition, shop costs, and namedesc bounds where covered. `huge meatball` and `huge chunk of meat` also resolve to `enormous meatball` in this local path. Generated `KELP_FROND` objects now also use the C `rnd(2)` quantity and food display metadata. Generated and wished `SLIME_MOLD` objects now store the current fruit id in `spe`, display through a small fruit registry, and keep C weight/nutrition/cost for the default/current-fruit path (`nethack-c/upstream/src/mkobj.c:941-943`, `nethack-c/upstream/src/objnam.c:747-774`, `nethack-c/upstream/src/objnam.c:4787-4867`, `nethack-c/upstream/src/objnam.c:5137-5139`). This still depends on local weight/display maps rather than the C `objects[]` table, and save/bones fruit-id remapping remains open.
...
8. **`readobjnam` matching is still local.** Unrecognized wishes no longer become arbitrary named weapons, exact `nothing`/`nil`/`none` declines now stay distinct from no-match retry, repeated failures randomize through the existing C-shaped path, and selected wizard trap plus broad terrain/furniture wishes now use the non-object result path. Generic object ranges, selected `called` namedesc paths, and scroll/spellbook `labeled`/`labelled` namedesc paths are covered locally. JS still lacks registry-backed C `wishymatch`, full object ranges, general namedesc lookup, alternate spellings, terrain side effects around liquid/object damage and wall-property qualifiers, broader denied-artifact handling, and full parser staging.

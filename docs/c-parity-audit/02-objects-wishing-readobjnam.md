...
- move the local requested-`spe` overlay into registry-backed class, `oc_charged`, luck, blessed/cursed, wand, and crystal-ball rules while preserving the covered bag-of-tricks, expensive-camera, tinning-kit, can-of-grease, magic-marker, and crystal-ball behavior
...
- replace the local weight maps behind final wished-object `owt` with registry-backed `weight()`; pancake/cram/kelp/royal-jelly/meatball/enormous-meatball/K/C ration weights plus bag-of-tricks, expensive-camera, tinning-kit, can-of-grease, magic-marker, and crystal-ball charged-tool weights are covered in local overlays, but the broader registry is still missing

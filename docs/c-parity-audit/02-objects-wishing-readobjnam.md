...
- `wishedSpeForItem` now has a narrow C-shaped class split: weapons, armor, weapon-tools, and charged rings use C's anti-abuse limits; wands and overlay-covered charged tools use local C metadata; crystal balls keep the wand-like negative `-1` cap, while bag of tricks and magic markers cap positive requested charges at generated charges and collapse negative requests to zero. It is still local policy because JS lacks a registry-backed `oc_charged`/class table for every object.
- `applyWishedQuantity` now follows the C shape for common cases: non-mergeable wishes such as boots, wands, meat rings, magic markers, bag of tricks, crystal balls, and spellbooks stay at one, plural spellings resolve to the base object where covered, pancake/cram/kelp/royal-jelly/meatball/enormous-meatball/K/C ration plural wishes get C-style two-object quantities, and candles/ammunition keep the C multigen caps. It is still local policy rather than full `objects[].oc_merge` metadata.
...
- move the local requested-`spe` overlay into registry-backed class, `oc_charged`, luck, blessed/cursed, wand, and crystal-ball rules while preserving the covered bag-of-tricks, magic-marker, and crystal-ball behavior
...
- replace the local weight maps behind final wished-object `owt` with registry-backed `weight()`; pancake/cram/kelp/royal-jelly/meatball/enormous-meatball/K/C ration weights plus bag-of-tricks, magic-marker, and crystal-ball charged-tool weights are covered in local overlays, but the broader registry is still missing

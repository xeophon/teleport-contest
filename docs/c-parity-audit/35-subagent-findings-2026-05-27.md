...
Potion-potion alchemy recipes and bad-mixture explosion/evaporation paths are covered in Audit 36, alchemy-explosion vapor effects are covered in Audit 37, and hard-landing projectile broken-potion vapor effects are covered in Audit 38. Full `poly_obj()` fidelity, worn-object side effects, self-potion/Klein-bottle handling, container-content deletion edge cases outside local object representation, shared `water_damage()` consolidation, direct `potionhit()` thrown/bash delivery, other broken-potion callers, non-`kn` `trycall()` prompt parity, water vapor gremlin/lycanthropy transformations, exact status-property mapping, and deeper potion description/type-call discovery remain separate work.
...
- Audit 36 covers the first potion-potion alchemy slice, Audit 37 adds bounded alchemy-explosion vapor effects, and Audit 38 adds hard-landing projectile broken-potion vapor effects. Remaining alchemy and vapor work is exact object-registry result metadata, fumbling drop behavior, complete altered-target shop repricing, direct `potionhit()` thrown/bash delivery, other broken-potion callers, non-`kn` `trycall()` prompt parity, and water vapor gremlin/lycanthropy transformations.

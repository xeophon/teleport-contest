...
**Phase 2 — Maintainability.** Target announced Nov 30; deadline
Dec 31, 2026 (00:00 UTC). The judges pick a goal for a "NetHack 5.1"
that requires you to update your JavaScript port. The exact goal is
intentionally open-ended — it might be a real upstream release after
5.0, a curated set of post-5.0 commits, a feature retrofit the
judges design, or some combination. The only commitment is that it
will be a real change you have to absorb. Your Phase 2 score is
parity against 5.1 **divided by** the complexity of the diff your
`js/` required to reach it, measured against your Phase 1 freeze tag.

Because the diff is anchored to your Phase 1 freeze, **the
architecture and readability of the codebase you submit at the end
of Phase 1 directly determines your Phase 2 ceiling.** Submissions
that arrive at Phase 1 with a clean, well-factored, easy-to-modify
codebase will have a strong advantage in Phase 2; submissions that
arrive with sprawling or unreadable code that resists change will
have to rewrite far more to retarget, and the diff penalty will
weigh heavily on the result.

Or, in NetHack terms: the player who memorized the layout of the
Castle does not necessarily survive Gehennom.
...
- **Phase 2 winner** — highest combined parity-divided-by-diff score
  ...
  leaderboard, and the satisfaction of having built a JavaScript
  port that handles change cleanly.

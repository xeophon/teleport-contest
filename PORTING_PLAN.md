...
1. Shop ledger foundation.
   ...
   - The next narrow C-backed gaps are remaining magic-bag loss-valuation/source/target cases after the covered paid-container put-in cleanup, shop-floor nested tip explosion, partly eaten food, and carried apply turn-cost/message slices, remaining merge/destruction edge cases including food-ration `oc_merge` compatibility, artifact touch side effects outside floor pickup/container take-out, less ordinary projectile ownership branches, full `obfree()`/container-aware `stolen_value()` debt naming, less ordinary `addtobill()`/quote positioning outside whole-container pickup, remaining costly-alteration paths outside the covered carried-object slices, and remaining `#pay` prompt validation/angry no-bill branch edge cases.
...
3. Add the ordinary food-ration pickup merge predicate before broader `oc_merge`: require same ration identity, BUC, `age`, name, and same-shop/unit-price proof for unpaid stacks while keeping tins, corpses, eggs, globs, and meat rings excluded.

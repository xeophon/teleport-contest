...
1. Continue ordinary victual runtime beyond the covered carried/floor delayed rows (`food ration`, `pancake`, `lembas wafer`, `cram ration`, and `tripe ration`) and covered delay-one apple/cream-pie/candy-bar/fortune-cookie/K-ration/C-ration rows: broader non-ordinary food branches, remaining delay-one special-effect rows, and remaining platform-specific apple message policy remain separate source-backed slices.

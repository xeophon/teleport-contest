...
2. Broaden merge parity beyond the completed simple `FOOD(...)` pickup path: the current covered rows are exact `food ration`, `pancake`, `lembas wafer`, `cram ration`, `cream pie`, `K-ration`, `C-ration`, `apple`, `orange`, `pear`, `melon`, `banana`, and `carrot` identity with C object-name compatibility, BUC/state checks, age averaging, no-charge normalization, C unit weights/costs where needed, and same-shop/unit-price proof for unpaid stacks; add corpses, tins, eggs, globs, fortune cookies, slime mold/custom fruits, meatballs, royal jelly, and broader `oc_merge` metadata only when their C fields, naming, and timers are modeled.

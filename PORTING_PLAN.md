...
- Latest compact/source note: `docs/c-parity-audit/345-generic-upward-dagger-dmgval-modifiers-2026-05-31.md`.
...
2. Direct object-hit and potion delivery.
   - Source notes: `docs/c-parity-audit/04-monsters-combat-pets.md` and latest compact note `docs/c-parity-audit/345-generic-upward-dagger-dmgval-modifiers-2026-05-31.md`.
...
4. Direct delivery work: broaden `potionhit()`/`toss_up()` and direct object-hit only through newly selected compact C-backed edges; direct potionhit paralysis chip-RNG and wakeup cleanup ordering is covered in audit 337, direct hero-thrown blinding-venom `hmon()` hits are covered in audit 339, direct acid-venom `hmon()` hits are covered in audit 340, direct cream-pie `hmon()` hits are covered in audit 341, wielded egg `hmon()` plus direct-object miss wakeups are covered in audit 342, generic upward plain tin opener plus clean +0 plain dagger `toss_up()` self-hit/landing is covered in audit 344, and non-artifact upward dagger `dmgval()` enchantment/erosion/fallback modifiers are covered in audit 345. Avoid broad monster lifecycle rewrites without tests.

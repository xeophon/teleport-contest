...
- Latest compact/source note: `docs/c-parity-audit/377-metallivorous-worn-ring-gone-2026-06-04.md`.
...
5. Registry/diet work: C `readobjnam` object ranges for `bag`, `lamp`, `candle`, `horn`, `sword`, `shoes`, `shirt`, `boots`, `gloves`/`gauntlets`, `cloak`, `shield`, `hat`, and `helm` are covered, positive `called` range-tail namedesc lookups, unresolved `called` random-class fallback, scroll/spellbook `labeled`/`labelled` namedesc, unknown-label class fallback, venom plural aliases, wizard-only `spe=1` policy, dragon armor RNG paths, exact local polearm wish metadata, and direct `mksobj()`/`mongets()` metadata for partisan/ranseur/spetum/glaive/lucern hammer are covered; remaining object registry work includes direct construction/wish metadata for halberd, bardiche, voulge, fauchard, guisarme, bill-guisarme, and bec de corbin, plus registry-backed consolidation of dragon armor and other parser-local metadata. Remaining diet metadata should be caller-led; digestible metallivorous worn-ring hand-state cleanup is covered in audit 377, while source-masked extrinsics, fullness, cursed, and strangulation cleanup remain open.

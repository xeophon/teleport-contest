...
2. Direct object-hit and potion delivery.
   ...
   - Remaining gaps include test-only adjacent common no-effect vapor coverage, shifted-vampire lethal revival, full burning-oil explosion collateral, identity-independent unknown-potion direct routing, and full `newcham()` target-selection/equipment fallout beyond the narrow polymorph-potion slice.
...
1. Broaden direct `potionhit()` delivery through the next compact C-backed edge: identity-independent unknown-potion direct routing and adjacent common no-effect vapor coverage. Full burning-oil explosion collateral belongs with broader explosion work, shifted-vampire lethal revival belongs with broader monster death lifecycle work, and full `newcham()` fidelity belongs with the monster lifecycle/equipment core.

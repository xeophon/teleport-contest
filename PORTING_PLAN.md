...
- Added focused C-style object-loss billing: food bites, tins, failed spellbook reads, carried fire destruction/ignition, hero-caused floor fire, box lock break, cream-pie splat, cursed wand backfire, magic-bag loss/scatter/blast paths, and container-aware projectile `stolen_value()` debt conversion.
...
- Improved C-style command/menu/payment details: multi-pickup partial-success ordering, burden/gold/scare-scroll menu handling, shop quote ordering/text, robbed-only `#pay`, itemized `#pay` row ordering/headings/affordability, debit-before-itemized settlement, and nonresident distance payment refusal.
...
1. Shop ledger foundation.
   ...
   - The next narrow C-backed gaps are non-ordinary magic-bag source/target cases, remaining inventory merge edge cases outside carried-bag partial put-in, remaining merge/destruction edge cases, artifact touch side effects outside floor pickup/container take-out, hard-landing projectile container-content impact before shop return/debt, full `obfree()`/container-aware `stolen_value()` debt naming, less ordinary `addtobill()`/quote positioning outside whole-container pickup, costly-alteration paths such as cursed charging `stripspe()`, and remaining `#pay` details such as legacy unpaid fallbacks and itemized prompt edge cases.

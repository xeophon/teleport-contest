...
1. Broaden direct `potionhit()` delivery only through a newly selected compact C-backed edge. Broader burning-oil terrain collateral still belongs with terrain work; floor objects, webs, hero slime cleanup, ordinary ice melting, and water/pool evaporation are covered, while fountains, doors/drawbridges, and broader hero-on-liquid fallout remain open. Other shifted-vampire death channels belong with monster death lifecycle work, and full `newcham()` fidelity belongs with the monster lifecycle/equipment core.
...
4. Continue projectile landing through floor-pile impact loss next; remote non-gold hero projectile seen-hole/trapdoor down-gate, pre-placement floor effects, and lava hard landing are covered. Keep gold-throw shipping, stairs/ladders, kicked-object shipping, and monster-thrown ordering separate.

...
- Added full top-level shop-bill limit parity: direct pickup, shop-floor container take-out, pickup/container merge probes, floor bag-of-tricks temporary rows, horn-created object billing, and split rows now leave objects free when C refuses a full `BILLSZ` bill instead of synthesizing field-only unpaid state.
...
7. Continue shop-bill cleanup past the top-level full-bill slice: mid-recursion container saturation, dummy `costly_alteration()` full-bill behavior, and remaining shared `addtobill()`/`subfrombill()` integration still need source-shaped helpers.

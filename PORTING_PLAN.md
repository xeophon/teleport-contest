...
- Earthquake floor-object liquid damage now recurses into containers along the
  ...
  places spilled contents on the liquid square, and routes spilled contents
  through a floor-effect mode instead of reusing top-level `fire_damage_chain()`.
  Spilled contents now cover the lava `flooreffects()`/`lava_damage()` slice:
  boulders can splash/fill the lava before later contents are processed, hard
  `obj_resists(0,0)` objects are preserved with Book of the Dead falling through
  for smoke feedback, ordinary material/organic contents burn up directly, and
  the remaining cases fall back to forced fire damage; water/moat keeps contents
  inside and recursively damages them unless the container is waterproof, with
  cursed waterproof containers using the C `rn2(3)` leakage gate. Direct smokes
  cover lava container burn/spill, lava direct burn, Book fallback smoke, nested
...
- Ice-box corpse age parity now covers generated ice-box contents,
  ...
  Bag-of-tricks and horn-of-plenty `#tip` specials now follow the C
  charge-drain shape: generated bag/horn tools store their `rn1(18, 3)`
  charges, bag tipping repeatedly consumes charges into monster creation and
  handles empty/no-visible-effect messages, horn tipping repeatedly creates

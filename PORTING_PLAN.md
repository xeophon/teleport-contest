...
3. Broaden merge parity beyond the completed simple `FOOD(...)` pickup path and floor/drop/projectile special-food gate: the current covered rows are exact `food ration`, `pancake`, `lembas wafer`, `cram ration`, `cream pie`, `fortune cookie`, `lump of royal jelly`, `meatball`, `enormous meatball`, `K-ration`, `C-ration`, `apple`, `orange`, `pear`, `melon`, `banana`, `carrot`, `kelp frond`, `slime mold`/current-fruit, `sprig of wolfsbane`, `clove of garlic`, and `eucalyptus leaf` identity with C object-name compatibility, BUC/state checks, age averaging, no-charge normalization, C unit weights/costs where needed, same-shop/unit-price proof for ordinary unpaid pickup merges, and C's conservative full-inventory billable shop-floor preflight rejection; floor/drop/projectile corpse/egg/tin stacks now reject mismatched species, hatching eggs, and revivable corpses. Next narrow merge targets are shared predicate reuse for pickup/container/level-generation paths, compatible non-timed eggs and tins in inventory, globs, and broader `oc_merge` metadata when their C fields, naming, and timers are modeled.

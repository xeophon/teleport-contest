...
1. Shop ledger foundation.
   ...
   - The next narrow C-backed gaps are non-ordinary magic-bag sources/targets, remaining lift preflight and inventory merge edge cases, remaining merge/destruction edge cases, generic floor-effect deletion ownership, full artifact touch side effects, and payment container/robbed-shop semantics.
...
1. Broaden shop-floor container take-out and tip beyond recursive billing into remaining lift preflight details, remaining inventory merge edge cases, and merge/destruction edge cases; continue magic-bag work through non-ordinary sources/targets and shared `obfree()`/`stolen_value()` debt naming.

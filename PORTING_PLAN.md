...
1. Broaden shop-floor container take-out and tip beyond recursive billing into remaining lift preflight details, remaining inventory merge edge cases outside carried-bag partial put-in, and merge/destruction edge cases; continue floor pickup through scare-scroll floor-style dust quote polish and container/contents quote edge cases, and continue magic-bag work through shop-floor cursed bag `#loot` loss, non-ordinary sources/targets, and shared `obfree()`/`stolen_value()` debt naming.

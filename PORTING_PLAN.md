...
1. Broaden direct `potionhit()` delivery only through a newly selected compact C-backed edge. Broader burning-oil terrain collateral still belongs with terrain work; floor objects, webs, hero slime cleanup, and ordinary ice melting are covered, while water/fountains/doors and hero-on-liquid fallout remain open. Other shifted-vampire death channels belong with monster death lifecycle work, and full `newcham()` fidelity belongs with the monster lifecycle/equipment core.

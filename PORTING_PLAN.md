...
1. Shop ledger foundation.
   ...
   - The next narrow C-backed gaps are remaining magic-bag loss-valuation/source/target cases after the covered paid-container put-in cleanup, shop-floor nested tip explosion, partly eaten food, carried apply turn-cost/message, and extracted-container nested-content valuation slices, remaining merge/destruction edge cases beyond the completed simple `FOOD(...)` pickup merge path and floor/drop/projectile plus pickup/container-takeout special-food gates, artifact touch side effects outside floor pickup/container take-out, less ordinary projectile ownership branches beyond same-shop paid/gold landing, full `obfree()`/container-aware `stolen_value()` debt naming, less ordinary `addtobill()`/quote positioning outside whole-container pickup, and remaining costly-alteration paths outside the covered carried-object slices.
...
3. Broaden merge parity beyond the completed simple `FOOD(...)` pickup path and floor/drop/projectile plus pickup/container-takeout special-food gates: the current covered rows are exact `food ration`, `pancake`, `lembas wafer`, `cram ration`, `cream pie`, `fortune cookie`, `lump of royal jelly`, `meatball`, `enormous meatball`, `K-ration`, `C-ration`, `apple`, `orange`, `pear`, `melon`, `banana`, `carrot`, `kelp frond`, `slime mold`/current-fruit, `sprig of wolfsbane`, `clove of garlic`, and `eucalyptus leaf` identity with C object-name compatibility, BUC/state checks, age averaging, no-charge normalization, C unit weights/costs where needed, same-shop/unit-price proof for ordinary unpaid pickup merges, and C's conservative full-inventory billable shop-floor preflight rejection; corpse/egg/tin paths now reject mismatched species, hatching eggs, and revivable corpses while allowing compatible non-timed eggs, same-species tins, and ordinary same-species corpses in inventory pickup/container-takeout with C-style name and age behavior. Next narrow merge targets are shared predicate reuse for `mklev.js` container/level-generation paths, monster inventory, globs, low-risk omitted food rows such as tripe/candy/meat stick, and broader `oc_merge` metadata when their C fields, naming, and timers are modeled.
...
6. Continue object timer work toward a central registry: the bounded egg timer cleanup now covers local hatch fields, so next timer slices should focus on shared timer serialization, migration, and catch-up instead of adding more independent object timer fields.

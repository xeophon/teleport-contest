...
2. Direct object-hit and potion delivery.
   ...
   - Remaining gaps include broader burning-oil terrain collateral, other shifted-vampire death channels outside the water branch, and full `newcham()` target-selection/equipment fallout beyond the narrow polymorph-potion slice.
...
1. Broaden direct `potionhit()` delivery only through a newly selected compact C-backed edge. Broader burning-oil terrain collateral belongs with terrain work, other shifted-vampire death channels belong with monster death lifecycle work, and full `newcham()` fidelity belongs with the monster lifecycle/equipment core.
2. Keep remaining stone-to-flesh work narrow: add only the statue/figurine vegetarian-or-non-flesh meatball row through existing replacement paths before touching animation, corpse fallback, contents transfer, timers, or broader `poly_obj()` fallout.

...
2. Direct object-hit and potion delivery.
   ...
   - Remaining gaps include bash delivery, non-`kn` `trycall()` prompts, exact visibility/discovery handling beyond covered direct/vapor cases, test-only adjacent common no-effect vapor coverage, shifted-vampire lethal revival, full burning-oil explosion collateral, and full `newcham()` target-selection/equipment fallout beyond the narrow polymorph-potion slice.
...
1. Broaden direct `potionhit()` delivery through the next compact C-backed edge: wielded-potion bash delivery or shared non-`kn` `trycall()` prompts. Full burning-oil explosion collateral belongs with broader explosion work, shifted-vampire lethal revival belongs with broader monster death lifecycle work, and full `newcham()` fidelity belongs with the monster lifecycle/equipment core.
2. Continue stone-to-flesh coverage through object resistance, boulder/gem/statue/figurine rows, and floor/beam/shop routing after each row has a source-backed test shape.

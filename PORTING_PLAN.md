...
1. Finish the remaining C `dopay()` polish after the scanner, `Pay whom?` prompt-validation, angry-not-robbed appeasement, and traditional itemized prompt slices: follow-up polish for `getpos()` monster/object cycling beyond the covered cursor movement, cancel, and target validation paths.

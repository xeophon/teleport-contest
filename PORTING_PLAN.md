...
- Added the carried ordinary victual runtime slices: non-rotten carried `food ration`, `pancake`, `lembas wafer`, and `cram ration` now use C-style first-bite `oeaten`/`nmod` timing, per-tick later bites, finish-time removal, one-time first-bite stack/shop billing, and race-adjusted hunger for lembas/cram while leaving floor, rotten/cursed, and delay-one food paths separate.
...
1. Continue ordinary victual runtime beyond the covered carried non-rotten delayed rows (`food ration`, `pancake`, `lembas wafer`, and `cram ration`): floor food, cursed/rotten ordinary delayed foods, exact interruption/resume messages, choking/full-warning behavior, apple, and fortune cookie remain separate source-backed slices.

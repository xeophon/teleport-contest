...
1. Shop ledger foundation.
   ...
   - The next narrow C-backed gaps are remaining magic-bag loss-valuation/source/target cases after the covered paid-container put-in cleanup, shop-floor nested tip explosion, partly eaten food, carried apply turn-cost/message, and extracted-container contained-gold valuation slices, remaining merge/destruction edge cases beyond the completed ordinary food-ration pickup merge path, artifact touch side effects outside floor pickup/container take-out, less ordinary projectile ownership branches, full `obfree()`/container-aware `stolen_value()` debt naming, less ordinary `addtobill()`/quote positioning outside whole-container pickup, and remaining costly-alteration paths outside the covered carried-object slices.
...
2. Broaden merge parity beyond the completed ordinary food-ration pickup path: keep the ration predicate limited to exact ration identity, C object-name compatibility, BUC/state checks, age averaging, no-charge normalization, and same-shop/unit-price proof for unpaid stacks; add corpses, tins, eggs, globs, and broader `oc_merge` metadata only when their C fields and timers are modeled.

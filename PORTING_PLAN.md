...
2. Add `ship_object()`-style fragile-object breakage for carried drops through seen holes/trapdoors after shop debt but before migration; leave `impact_drop()` floor-pile migration unchanged because C does not run `breaktest()` there.
...
5. Move payment toward a complete C `dopay()` model, covering selected-item insufficient-funds wording, multi-shopkeeper `Pay whom?`, angry-adjacent priority, resident/visible-shopkeeper scan differences, and remaining itemized prompt edge cases.
6. Continue wish finalization by replacing local parser/finalizer tables with registry-backed `oc_merge`, `oc_charged`, `oc_nowish`, canonical weight, and artifact provenance rules; next small rows include meat ring non-mergeability/weight, candle merge/weight, horn-of-plenty metadata, and wand-of-wishing charge tests.

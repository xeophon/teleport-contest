...
- Latest compact/source note: `docs/c-parity-audit/203-direct-melee-passive-object-erosion-2026-05-30.md`.
...
1. Shared shop ownership helpers.
   ...
   - Compact candidates: remaining magic-bag valuation/discovery edges, remaining monster-thrown passive-object follow-ups, floor-statue saved-traits/cant-revive rows, costly-alteration paths, and remaining stone-to-flesh/object-polymorph lifecycle rows.
...
2. Projectile and migration work: seen-hole/trapdoor shipping, projectile/gold/monster-thrown down-stairs/down-ladder/special-stairs route metadata, carried non-gold plus carried-gold command/helper down-gate shipping, kicked-object floor selection/down-gate shipping, queued delivery silent breakage/arrival stacking, hit-only monster-thrown missile mulch, monster-thrown rust/fire/acid/corrosion passive-object erosion, production sling rock/plain dagger/crude dagger `ohit` threading, monster-thrown disenchanter `AD_ENCH` drain, direct hero melee `AD_ENCH` drain, direct hero melee `AD_RUST`/`AD_CORR`/`AD_FIRE`/`AD_ACID` passive-object erosion, hero/polyself passive-object targets, production dart hit landing, and plain +0 production launcher-arrow hit/miss landing are covered; next launcher-arrow slice should be unblessed/uncursed/ungreased/uneroded `+1` arrows on nonlethal hit and miss, leaving blessed, eroded, greased, cursed, and lethal arrow persistence separate until replay/bones ordering is verified.

...
- Added the ordinary victual runtime slices: carried and floor `food ration`, `pancake`, `lembas wafer`, and `cram ration` now use C-style first-bite `oeaten`/`nmod` timing, per-tick later bites, finish-time removal, one-time first-bite stack/shop billing, race-adjusted hunger for lembas/cram, cursed/age-rotten first-bite handling, full-warning/finally-finished state, and same-object interruption/resume messages/state including satiated continue prompts. Delay-one carried/floor apple and fortune cookie now also use the first-bite stack/shop billing path, finish without an occupation, and preserve cursed-apple and fortune-cookie post-effects.
...
1. Continue ordinary victual runtime beyond the covered carried/floor delayed rows (`food ration`, `pancake`, `lembas wafer`, and `cram ration`) and covered delay-one apple/fortune-cookie rows: fatal choking/vomit recovery, broader non-ordinary floor food branches, and remaining platform-specific apple message policy remain separate source-backed slices.

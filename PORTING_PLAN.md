...
1. Shop ledger foundation.
   ...
   - The next narrow C-backed gaps are floor-effect used-up preservation for ordinary drops and tipped shop goods, remaining magic-bag loss-valuation/source/target cases beyond paid-container put-in cleanup, shop-floor nested tip explosions, partly eaten food, and carried apply turn-cost/messages, remaining merge/destruction edge cases, artifact touch side effects outside floor pickup/container take-out, less ordinary projectile ownership branches, full `obfree()`/container-aware `stolen_value()` debt naming, less ordinary `addtobill()`/quote positioning outside whole-container pickup, remaining costly-alteration paths outside the covered carried-object slices, and remaining `#pay` target-selection/prompt edge cases such as selected-item insufficient-funds wording.
...
1. Keep generic `fire_damage()` floor catch-light unbilled, and wire used-up bill preservation only into source-confirmed object-free paths such as ordinary carried drops and tipped shop goods destroyed by terrain.
2. Move payment toward a complete C `dopay()` model, starting with selected-item insufficient-funds wording, then multi-shopkeeper `Pay whom?`, angry-adjacent priority, resident/visible-shopkeeper scan differences, and remaining itemized prompt edge cases.
3. Continue magic-bag work through remaining source/target ordering, loss valuation, and shared `obfree()`/`stolen_value()` debt naming after the paid-container put-in, shop-floor nested tip, carried apply turn-cost/message, and stale take-out/tip billing slices.
...
5. Continue wish finalization by replacing local parser/finalizer tables with registry-backed `oc_merge`, `oc_charged`, `oc_nowish`, canonical weight, and artifact provenance rules; next small rows include meat ring non-mergeability/weight, candle merge/weight, horn-of-plenty metadata, and wand-of-wishing charge tests.

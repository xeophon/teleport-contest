...
2. Direct object-hit and potion delivery.
   ...
   - Remaining gaps include broader burning-oil terrain collateral beyond covered floor objects/webs/slime cleanup/ice melting, other shifted-vampire death channels outside the water branch, and full `newcham()` target-selection/equipment fallout beyond the narrow polymorph-potion slice.
...
4. Monster placement, scheduler, and combat cores.
   - Source notes: `docs/c-parity-audit/04-monsters-combat-pets.md`, `docs/c-parity-audit/92-subagent-findings-2026-05-28.md`, `docs/c-parity-audit/95-subagent-findings-2026-05-28.md`, and `docs/c-parity-audit/98-subagent-findings-2026-05-28.md`.
...
1. Broaden direct `potionhit()` delivery only through a newly selected compact C-backed edge. Broader burning-oil terrain collateral still belongs with terrain work; floor objects, webs, hero slime cleanup, and ordinary ice melting are covered, while water/pool evaporation is now audited as the next small terrain slice and fountains/doors plus hero-on-liquid fallout remain open. Other shifted-vampire death channels belong with monster death lifecycle work, and full `newcham()` fidelity belongs with the monster lifecycle/equipment core.
...
3. Continue registry-backed cleanup for merge/wish/charged-tool metadata. Polyself diet metadata is covered for stone-to-flesh smell, tripe, and tin eating; remaining diet metadata should be caller-led, starting with pet food or non-food metallivorous `#eat`.

...
5. Continue wish finalization by replacing local parser/finalizer tables with registry-backed `oc_merge`, `oc_charged`, `oc_nowish`, canonical weight, and artifact provenance rules; the concrete food rows and charged-tool metadata for bag of tricks, magic marker, and crystal ball are covered, so next small rows should focus on broader `oc_nowish`, artifact provenance, other charged tools, and generated-weight/factory unification.

...
2. Continue shop-bill cleanup with full-bill `BILLSZ` parity: direct pickup, pickup merge preflight, shop-floor container takeout, floor bag-of-tricks `#tip` temporary rows, and split-row creation should leave objects free when C would refuse to add a bill row.
...
7. Continue object timer work toward a central registry: the bounded egg timer cleanup now covers local hatch fields, so next timer slices should focus on contained due egg expiration, split timer identity, shared timer serialization, migration, and catch-up instead of adding more independent object timer fields.

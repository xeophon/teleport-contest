...
2. Object registry and canonical object factory.
   ...
   - Continue replacing the independent wish parser with C-shaped matching, full `objects[].oc_merge`/`oc_charged` metadata beyond the local food and charged-tool overlays, explicit "nothing" handling, artifact provenance, remaining non-wishable filtering, and registry-backed object finalization rules.
...
5. Continue wish finalization by replacing local parser/finalizer tables with registry-backed `oc_merge`, `oc_charged`, `oc_nowish`, canonical weight, and artifact provenance rules; the concrete food rows and charged-tool metadata for bag of tricks, expensive camera, tinning kit, can of grease, magic marker, and crystal ball are covered, so next small rows should focus on broader `oc_nowish`, artifact provenance, charged instruments/special charged tools, and generated-weight/factory unification.

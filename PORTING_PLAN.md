...
1. Shop ledger foundation.
   ...
   - The next narrow C-backed gaps are remaining magic-bag loss-valuation/source/target cases after the covered paid-container put-in cleanup, shop-floor nested tip explosion, partly eaten food, carried apply turn-cost/message, and extracted-container nested-content valuation slices, remaining merge/destruction edge cases beyond the completed ordinary delayed-food pickup merge path, artifact touch side effects outside floor pickup/container take-out, less ordinary projectile ownership branches including paid same-shop projectile `sellobj()`, full `obfree()`/container-aware `stolen_value()` debt naming, less ordinary `addtobill()`/quote positioning outside whole-container pickup, and remaining costly-alteration paths outside the covered carried-object slices.
...
1. Continue magic-bag work through remaining source/target ordering, loss valuation, and shared `obfree()`/`stolen_value()` debt naming after the paid-container put-in, shop-floor nested tip, carried apply turn-cost/message, extracted-container nested-content valuation, and stale take-out/tip billing slices.
2. Broaden merge parity beyond the completed ordinary delayed simple-food pickup path: the current covered rows are exact `food ration`, `pancake`, `lembas wafer`, and `cram ration` identity with C object-name compatibility, BUC/state checks, age averaging, no-charge normalization, C unit weights, and same-shop/unit-price proof for unpaid stacks; add corpses, tins, eggs, globs, custom fruits, meatballs, royal jelly, and broader `oc_merge` metadata only when their C fields, naming, and timers are modeled.
3. Finish remaining `sellobj()` follow-ups: complete recursive `subfrombill()` integration, paid same-shop projectile and thrown-gold landing branches, less ordinary projectile ownership branches, and shared-helper integration for less ordinary object transfers.

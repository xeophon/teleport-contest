...
- Added the first carried ordinary victual runtime slice: exact non-rotten carried `food ration` now uses C-style first-bite `oeaten`/`nmod` timing, per-tick later bites, finish-time removal, and one-time first-bite stack/shop billing while leaving floor food and other ordinary foods unchanged.
...
1. Broaden ordinary victual runtime from exact carried non-rotten `food ration` to other delayed non-special carried foods only after replay-sensitive prompt/RNG paths are protected: pancake, lembas wafer, and cram ration are the next source-backed candidates; floor food, rotten rations, exact interruption/resume messages, choking/full-warning behavior, apple, and fortune cookie remain separate.

...
- Added the ordinary victual runtime slices: carried and floor `food ration`, `pancake`, `lembas wafer`, and `cram ration` now use C-style first-bite `oeaten`/`nmod` timing, per-tick later bites, finish-time removal, one-time first-bite stack/shop billing, race-adjusted hunger for lembas/cram, and cursed/age-rotten first-bite handling. Delay-one carried/floor apple and fortune cookie now also use the first-bite stack/shop billing path, finish without an occupation, and preserve cursed-apple and fortune-cookie post-effects.

...
export async function moveloop_core() {
    ...
    while (g._pending_time_passed
        ...
        if (g._search_pending_count > 0) {
            ...
            // The check also belongs to the occupation branch of moveloop2
            // (allmain.c:485), which only fires for continuation ticks: the
            // press's first search is the direct rhack command action
            // (cmd.c:3728-3748 sets the "searching" occupation before
            // running dosearch() itself), so no monster_nearby check runs
            // between the first search and that turn's monster phase —
            // hence the _search_ticks_this_press > 1 gate on the arming.

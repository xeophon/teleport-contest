...
function monsterTrappedTrapTurn(mon) {
    ...
    if (process.env.DBG7542) console.error('DBG-MINTRAP', getRngLog().length, mon.data?.name, mon.mx, mon.my, trap.ttyp);
...
export async function moveloop_core() {
    ...
    while (g._pending_time_passed
         ...
         * This immediate check catches the intervene-mid-tail spawn (slice/cont9150);
         * the deferred pass-end check (g._search_stop_check_after_monsters below)
         * handles the C tick-order case and is skipped here because this block
         * clears _search_pending_count first. */
